...
class App extends React.PureComponent {
  ...
  render() {
    const SearchPanel = (
      <Search
        animate={this.state.removeLandingPage}
        defaultValue={this.state.searchText}
        onclickLoad={this.loadContent}
    ...
    return (
      <div ref={this.iScroll} style={{ height: "100vh", overflow: "auto" }}>
        <Header
          text={"Recipe Search"}
          animate={this.state.removeLandingPage}
          subText={"A search engine to find their recipes by the ingredients."}>
          {this.state.removeLandingPage && (
            <Search
              animate={false}
              defaultValue={this.state.searchText}
              onclickLoad={this.loadContent}
